...
import { useUser, useUpdatePreferences, useUpdateName, useUploadBanner, useDeleteBanner } from "../hooks/useUsers";
...
const NAME_COLORS = [
  "#0f172a", "#0ea5e9", "#8b5cf6", "#10b981",
  "#f43f5e", "#f59e0b", "#6366f1", "#ec4899",
  "#14b8a6", "#fb923c", "#a3e635", "#64748b",
];

const BANNER_COLORS = [
  "#0f172a", "#1e293b", "#1a1a2e", "#2d1b69",
  "#0c4a6e", "#14532d", "#7c2d12", "#1c1917",
  "#0369a1", "#064e3b", "#4c0519", "#292524",
];
  ...
  return (
    <div className="flex flex-wrap gap-2 items-center">
      <button
        ...
        title="Automatisch"
      >
        {value === "" && <Check size={10} className="m-auto text-white drop-shadow" />}
      </button>
      ...
      {presets.map((c) => (
        <button
          ...
          title={c}
        >
          {value === c && <Check size={10} className="m-auto text-white drop-shadow" />}
        </button>
      ))}
      ...
      <div className="relative h-7 w-7">
        ...
        <div className="h-7 w-7 rounded-full bg-[conic-gradient(from_0deg,#f43f5e,#f59e0b,#84cc16,#0ea5e9,#8b5cf6,#f43f5e)] flex items-center justify-center shadow-sm">
          <span className="text-[9px] font-black text-white drop-shadow leading-none">+</span>
        </div>
      </div>
    </div>
  );
      ...
      <div className="px-6 pb-6 -mt-10">
        <div
          className={`h-[76px] w-[76px] rounded-full border-4 border-white dark:border-slate-900 overflow-hidden mb-3 shadow-lg ${
            !hasAvatar ? `bg-gradient-to-br ${avatarColor(user.name)} flex items-center justify-center` : ""
          }`}
          style={!hasAvatar && displayColor ? { backgroundColor: displayColor, backgroundImage: "none", borderColor: undefined } : undefined}
        >
          {hasAvatar ? (
            <img src={user.avatar_url} alt={user.name} className="h-full w-full object-cover" onError={() => setImgErr(true)} />
          ) : (
            <span className="text-2xl font-black text-white m-auto block text-center leading-[76px]">{user.name[0].toUpperCase()}</span>
          )}
        </div>

        <p className="text-xl font-black text-slate-900 dark:text-white" style={nameStyle}>{user.name}</p>
        {displayPronouns && <p className="text-sm text-slate-400 mt-0.5">{displayPronouns}</p>}
          ...
          <div className="mt-4 space-y-2 border-t border-slate-100 dark:border-slate-800 pt-4">
            ...
            {user.phone_number && (
              <div className="flex items-center gap-2 text-sm">
                ...
                <a href={`tel:${user.phone_number}`} className="text-sky-500">{user.phone_number}</a>
              </div>
            )}
...
export function ProfilePage() {
  ...
  async function onBannerConfirm(blob: Blob, isGif: boolean) {
    setCropOpen(false);
    try {
      await uploadBannerMutation.mutateAsync({ blob, mimeType: isGif ? "image/gif" : "image/jpeg" });
  ...
  const bannerStyle = getBannerStyle(displayBanner, user?.banner_url ?? undefined);
  const nameStyle: React.CSSProperties = {
    color: displayColor || undefined,
    fontFamily: displayFont && displayFont !== "default" ? FONT_MAP[displayFont] : undefined,
  };
  ...
  if (!user) {
    return (
      <div className="flex min-h-[100dvh] flex-col items-center justify-center gap-3 bg-slate-50 dark:bg-slate-950">
        ...
        <button onClick={() => navigate(-1)} className="flex items-center gap-1.5 text-xs text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 transition-colors">
  ...
  if (!isOwn) {
    return (
      <div className="min-h-[100dvh] bg-slate-50 dark:bg-slate-950">
        ...
        >
          <button onClick={() => navigate(-1)} className="flex h-8 w-8 items-center justify-center rounded-lg text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 hover:text-slate-700 dark:hover:text-white transition-colors">
            <ArrowLeft size={17} />
          </button>
          <p className="text-sm font-bold text-slate-900 dark:text-white">{user.name}</p>
        </div>
        <div className="mx-auto max-w-sm px-4 py-6" style={{ paddingBottom: "max(2rem, env(safe-area-inset-bottom, 0px))" }}>
  ...
  return (
    <div className="min-h-[100dvh] bg-slate-50 dark:bg-slate-950">
      ...
      >
        ...
        {/* Banner */}
        <div className="relative h-[160px] sm:h-[200px] w-full" style={bannerStyle}>
          ...
          <div className="mx-auto max-w-4xl px-5 sm:px-8 pb-5">
            ...
            <div className="-mt-[44px]">
              <div className="relative inline-block">
                <div
                  ...
                  style={!hasAvatar && draftColor ? { backgroundColor: draftColor, backgroundImage: "none" } : undefined}
                >
                  {hasAvatar ? (
                    <img src={user.avatar_url} alt={user.name} className="h-full w-full object-cover" onError={() => setAvatarImgErr(true)} />
                  ) : (
            ...
            <div className="mt-3 flex items-center justify-between gap-3">
              <div className="min-w-0">
                <h1 className="text-xl font-black text-slate-900 dark:text-white leading-tight truncate" style={nameStyle}>
                  {user.name}
                </h1>
                {draftPronouns && (
                  <p className="text-sm text-slate-400 dark:text-slate-500 mt-0.5">{draftPronouns}</p>
                )}
              </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_280px] gap-5 items-start">
          ...
          <div className="space-y-5">
            ...
            <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35, delay: 0.05 }}>
              <Card title="Persoonlijke info" subtitle="Pas hier je persoonlijke gegevens aan.">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">

                  {/* Naam */}
                  <Field label="Weergavenaam" hint="Historische data blijft onder de oude naam staan.">
                    <div className="flex gap-2">
                      ...
                      <button
                        onClick={onRename}
                        disabled={!draftName.trim() || !!nameError || draftName.trim() === user.name || renameMutation.isPending}
                  ...
                  {/* Voornaamwoorden */}
                  <Field label="Voornaamwoorden" hint="Zichtbaar op je profielkaart.">
                    <input
                      type="text"
                      maxLength={40}
                  ...
                  {/* Telefoonnummer */}
                  <Field label="Telefoonnummer" hint="Zichtbaar voor andere deelnemers." className="sm:col-span-2">
            ...
            <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35, delay: 0.1 }}>
              <Card title="Weergave" subtitle="Pas de visuele stijl van je profiel aan.">
                <div className="space-y-6">

                  {/* Naamkleur */}
                  <Field label="Naamkleur">
                    <ColorPicker value={draftColor} onChange={setDraftColor} presets={NAME_COLORS} />
                    {draftColor && (
                      <div className="mt-2.5 inline-flex items-center rounded-lg border border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/50 px-3 py-1.5">
                        <span
                          ...
                          style={{ color: draftColor, fontFamily: draftFont !== "default" ? FONT_MAP[draftFont] : undefined }}
                        >
                          {user.name}
                        </span>
                      </div>
                        ...
                        return (
                          ...
                          >
                            <span
                              ...
                              style={{ fontFamily: value !== "default" ? FONT_MAP[value] : undefined }}
                            >
                              Aa
                            </span>
                            <span className={`text-[9px] font-bold uppercase tracking-wide ${isActive ? "text-sky-500" : "text-slate-400"}`}>
                              {label}
                            </span>
                          </button>
                        );
            ...
            <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35, delay: 0.16 }}>
              <Card title="Bannerafbeelding">
                ...
                {user?.banner_url ? (
                  <>
                    {/* Preview */}
                    <div className="relative overflow-hidden rounded-xl mb-3 border border-slate-100 dark:border-slate-800" style={{ aspectRatio: "3/1" }}>
