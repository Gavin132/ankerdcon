 ...
 */
export function TicketStack({ trips, meals, users, myNames, justJoinedId, onJoin, onLeave, onManage }: TicketStackProps) {
  ...
  const peekLabel = (t: Trip) => {
    ...
    return (
      <>
        ...
        <span className="truncate">{t.title} · {t.dateRange}</span>
      </>
    );
  };
